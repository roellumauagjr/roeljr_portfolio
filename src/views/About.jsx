...
const AboutView = ({ SectionHeader, GlassCard, isSwitchOn, EDUCATION, ORGS }) => {
  const hasData = EDUCATION && ORGS && EDUCATION.length > 0 && ORGS.length > 0;
          ...
          <TiltCard>
            <GlassCard className={`p-8 md:p-12 h-full ${isSwitchOn ? 'hover:shadow-[0_40px_80px_rgba(34,197,94,0.1)]' : 'hover:shadow-[0_40px_80px_rgba(220,38,38,0.1)]'} transition-all duration-700`}>
              ...
              <p className="text-lg text-[#121212]/70 leading-relaxed font-medium mb-6">
                {isSwitchOn 
                  ? "I am a Computer Science undergraduate with proven excellence in web development and UI/UX design. My journey in tech extends beyond the code editor; it is deeply enriched by my background as a Layout Artist and my active roles in campus public relations and documentation."
              ...
              <p className="text-lg text-[#121212]/70 leading-relaxed font-medium">
                {isSwitchOn
                  ? "This blend of creative design and technical problem-solving means I don't just build functional web applications; I thrive on crafting visually compelling, user-centric experiences and effectively collaborating with teams to deliver tangible results."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            ...
            <TiltCard>
              <GlassCard className={`p-8 md:p-10 h-full ${isSwitchOn ? 'hover:shadow-[0_40px_80px_rgba(34,197,94,0.1)]' : 'hover:shadow-[0_40px_80px_rgba(220,38,38,0.1)]'} transition-all duration-700`}>
                <h3 className="text-2xl font-black mb-8 flex items-center gap-3">
                  <span className={`p-3 ${isSwitchOn ? 'bg-green-600/10 text-green-600' : 'bg-red-600/10 text-red-600'} rounded-full`}><GraduationCap size={24} /></span>
                ...
                <div className="space-y-6">
                  {EDUCATION.map((edu, i) => (
                    <motion.div 
                      key={i}
                      whileHover={{ x: 10 }}
                      className={`relative pl-6 border-l-2 border-[#121212]/10 ${isSwitchOn ? 'hover:border-green-600' : 'hover:border-red-600'} transition-all duration-300 cursor-pointer`}
                    >
                      <div className={`absolute -left-[9px] top-1 w-4 h-4 bg-white border-4 ${isSwitchOn ? 'border-green-600' : 'border-red-600'} rounded-full`} />
                      <h4 className="text-lg font-bold">{edu.degree}</h4>
                      <p className="text-[#121212]/60 font-medium">{edu.school}</p>
                      <p className="text-[#121212]/50 text-sm font-medium mt-1">{edu.period}</p>
                      {edu.details && <p className={`text-sm font-bold ${isSwitchOn ? 'text-green-600' : 'text-red-600'} mt-1 uppercase tracking-wider`}>{edu.details}</p>}
                    </motion.div>
                  ))}
                </div>
              </GlassCard>
          ...
          <ScrollReveal delay={0.2}>
            <TiltCard>
              <GlassCard className={`p-8 md:p-10 h-full ${isSwitchOn ? 'hover:shadow-[0_40px_80px_rgba(34,197,94,0.1)]' : 'hover:shadow-[0_40px_80px_rgba(220,38,38,0.1)]'} transition-all duration-700`}>
                <h3 className="text-2xl font-black mb-8 flex items-center gap-3">
                  <span className={`p-3 ${isSwitchOn ? 'bg-green-600/10 text-green-600' : 'bg-red-600/10 text-red-600'} rounded-full`}><Users size={24} /></span>
                  <TextReveal text="Organizations" delay={0.4} />
                </h3>
                <ul className="space-y-4 font-bold text-[#121212]/80">
                  {ORGS.map((item, i) => (
                    <Magnetic key={i} strength={0.05}>
                      <li className={`flex flex-col bg-white/50 backdrop-blur-md p-4 rounded-2xl border border-white/80 ${isSwitchOn ? 'hover:border-green-600' : 'hover:border-red-600'} hover:bg-white transition-all duration-300 shadow-sm cursor-pointer group`}>
                        <span className={`${isSwitchOn ? 'text-green-600' : 'text-red-600'} text-xs tracking-widest uppercase mb-1 group-hover:scale-105 origin-left transition-transform`}>{item.role}</span>
                        <span className="group-hover:translate-x-1 transition-transform">{item.org}</span>
                        {item.period && <span className="text-[#121212]/40 text-xs mt-1">{item.period}</span>}
                      </li>
                    </Magnetic>
                  ))}
                </ul>
              ...
              <GlassCard className={`p-8 md:p-10 h-full ${isSwitchOn ? 'hover:shadow-[0_40px_80px_rgba(34,197,94,0.1)]' : 'hover:shadow-[0_40px_80px_rgba(220,38,38,0.1)]'} transition-all duration-700`}>
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <p className={`text-sm font-bold ${isSwitchOn ? 'text-green-600' : 'text-red-600'} mt-1 uppercase tracking-wider`}>Expected 2027</p>
                  </motion.div>
                </div>
              </GlassCard>
            </TiltCard>
